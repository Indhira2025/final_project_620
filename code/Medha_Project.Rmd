...
```{r Master Data}
master_data <- tidyr::expand_grid(
  date = unique(all_dates$date),
  state = unique(cleaned_population$state)
) %>%
  mutate(
    mmwr_year = epiyear(date),
    mmwr_week = epiweek(date)
...
master_data <- master_data %>%
  left_join(
    cleaned_population %>% dplyr::select(state, state_name, Year, Population, region_name),
...
master_data <- master_data %>%
  left_join(deaths_clean %>% rename(date = start_date), 
...
```{r Fatality Rate- adding wave data to clean data sets and aggregate by wave}

assign_wave <- function(date) {
  case_when(
    date >= as_date("2020-03-01") & date <= as_date("2021-02-28") ~ "1st Wave: Original",
    date >= as_date("2021-01-01") & date <= as_date("2021-06-30") ~ "2nd Wave: Alpha",
    date >= as_date("2021-07-01") & date <= as_date("2021-11-30") ~ "3rd Wave: Delta",
...
deaths_clean <- deaths_clean %>%
  mutate(wave = assign_wave(start_date)) %>%
  filter(!is.na(wave))
...
kable(wave_summary, digits = 3, caption = "COVID-19 CFR and Hospitalization Rate by Variant Wave")
wave_summary <- wave_summary[-nrow(wave_summary), ]
...
vax_clean <- vax_clean %>%
  mutate(wave = assign_wave(date)) %>%
  filter(!is.na(wave))

vax_by_wave <- vax_clean %>%
  group_by(wave) %>%
...
ggplot() +
  ...
  scale_color_manual(values = c("Cases per 100k" = "navy", "Hospitalizations per 100k" = "firebrick")) +
