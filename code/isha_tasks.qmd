...
```{r}
pre_covid_death1 %>% mutate(mmwr_week = as.integer(mmwr_week), total_deaths = as.numeric(total_deaths))

predict(lm(total_deaths ~ state + mmwr_week, data = pre_covid_death1), covid_deaths1 %>% select(state, mmwr_week))
```
